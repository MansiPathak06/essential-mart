...
const EMPTY_FORM = { name: '', slug: '', group_label: 'Men', image_url: '', sort_order: 0, is_active: true };
...
export default function AdminCategoriesPage() {
    ...
    const stats = {
        ...
        men: categories.filter(c => c.group_label === 'Men').length,
        women: categories.filter(c => c.group_label === 'Women').length,
        kids: categories.filter(c => c.group_label === 'Kids').length,
    };
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                    ...
                    { label: 'Men', value: stats.men, color: 'bg-amber-50 text-amber-700' },
                    { label: 'Women', value: stats.women, color: 'bg-rose-50 text-rose-700' },
                    { label: 'Kids', value: stats.kids, color: 'bg-teal-50 text-teal-700' },
                ].map(s => (
            ...
            <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-stone-100">
                        {filtered.length === 0 ? (
                            <tr><td colSpan={7} className="text-center py-10 text-stone-400">Koi category nahi</td></tr>
                        ) : filtered.map(cat => (

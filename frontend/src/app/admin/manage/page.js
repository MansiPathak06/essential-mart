...
import { Edit3, Eye, RefreshCw, Search, Trash2, X } from "lucide-react";
import {
  API_BASE,
  adminFetch,
  formatMoney,
...
export default function ManageProductsPage() {
  ...
  const filtered = useMemo(() => {
    return products.filter((product) => {
      ...
      const matchesSearch = `${product.name} ${product.category} ${product.sub_category}`
        .toLowerCase()
      ...
      {!loading && !error && (
        <div className="space-y-2">
          {filtered.map((product) => (
            <div
              key={product.id}
              className="grid gap-3 rounded-lg border border-stone-200 p-3 md:grid-cols-[56px_1fr_auto] md:items-center"
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-amber-300 text-amber-800 hover:bg-amber-50"
                  title="Edit product"
                >
                  <Edit3 size={16} />
                </button>
                <button
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-300 text-red-700 hover:bg-red-50"
...
function ProductThumb({ product }) {
  ...
  if (!src) {
    return <div className="h-14 w-14 rounded-lg bg-stone-100" />;
  }
  ...
  return (
    <Modal onClose={onClose}>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Field label="Discounted Price">
            <input
              ...
              onChange={(event) => setForm((current) => ({ ...current, discountedPrice: event.target.value }))}
              className={inputClass}
            />
          </Field>
        </div>
      ...
      <div className="flex justify-end gap-2 border-t border-stone-200 p-4">
        <button type="button" onClick={onClose} className="rounded-lg border border-stone-300 px-4 py-2 text-sm font-bold">
          Cancel
        </button>
        <button
          type="button"
    ...
    <Modal onClose={onClose}>
      <div className="p-5">
        ...
        <p className="mt-2 text-sm font-semibold text-stone-600">
          Product #{product.id} will be removed from the database.
        </p>
...
const inputClass =
  "w-full rounded-lg border border-stone-300 px-3 py-2 text-sm font-semibold text-stone-950 outline-none focus:border-stone-950";

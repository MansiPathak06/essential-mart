...
import { CheckCircle, MapPin, CreditCard, Truck, ChevronRight, Plus, Loader2 } from 'lucide-react';
...
export default function CheckoutPage() {
    ...
    const mrpTotal = Math.round(items.reduce((sum, i) => sum + ((i.product.originalPrice || i.product.discountedPrice) * i.quantity), 0));
    const payableTotal = Math.round(items.reduce((sum, i) => sum + (i.product.discountedPrice * i.quantity), 0));
    const savings = mrpTotal - payableTotal;

    useEffect(() => {
        const token = localStorage.getItem('token');
        if (!token) { router.push('/login'); return; }
        if (items.length === 0 && !ordered) { router.push('/cart'); return; }
    ...
    const handlePlaceOrder = async () => {
        if (!selectedAddress) { alert('Delivery address select karo!'); return; }
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
            <div className="bg-white rounded-2xl shadow-sm p-10 max-w-md w-full text-center">
                ...
                <div className="flex gap-3">
                    <Link href="/orders" className="flex-1 bg-black text-white py-3 rounded-lg text-sm font-bold text-center hover:bg-gray-800 transition">
                        Track Order
                    </Link>
                    <Link href="/" className="flex-1 border border-gray-200 py-3 rounded-lg text-sm font-bold text-center hover:bg-gray-50 transition">
                        Continue Shopping
                    </Link>
                </div>
            </div>
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    <div className="flex-1">
                        ...
                        {step === 0 && (
                            <div className="bg-white rounded-2xl shadow-sm p-6">
                                ...
                                {showForm && (
                                    <form onSubmit={handleAddAddress} className="mb-6 p-4 bg-gray-50 rounded-xl grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        <h3 className="col-span-full text-sm font-bold text-gray-700">New Address</h3>
                                        {[
                                            ...
                                            <input key={f.name} required placeholder={f.placeholder}
                                        ...
                                        <div className="col-span-full flex gap-3">
                                            <button type="submit" className="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 transition">Save</button>
                                            <button type="button" onClick={() => setShowForm(false)}
                                                className="border px-5 py-2 rounded-lg text-sm text-gray-500 hover:bg-gray-50 transition">Cancel</button>
                                        </div>
                                    </form>
                                )}
                                ...
                                {addresses.length === 0 && !showForm ? (
                                    <div className="text-center py-8 text-gray-400">
                                        ...
                                        <p className="text-sm">Koi address saved nahi hai</p>
                                        <button onClick={() => setShowForm(true)} className="mt-3 text-blue-600 text-sm font-bold hover:underline">+ Add Address</button>
                                    </div>
                                ) : (
                                        ...
                                        {addresses.map(addr => (
                                            <div key={addr.id} onClick={() => setSelectedAddress(addr.id)}
                                                ...
                                                <div className="flex items-start gap-3">
                                                    <div className={`w-4 h-4 rounded-full border-2 mt-0.5 flex-shrink-0 ${selectedAddress === addr.id ? 'border-black bg-black' : 'border-gray-300'}`} />
                                                    <div>
                                ...
                                <button onClick={() => { if (!selectedAddress) { alert('Address select karo!'); return; } setStep(1); }}
                            ...
                            <div className="bg-white rounded-2xl shadow-sm p-6">
                                ...
                                <div className="space-y-3">
                                    {PAYMENT_METHODS.map(pm => (
                                        <div key={pm.id} onClick={() => setPaymentMethod(pm.id)}
                                            ...
                                            <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${paymentMethod === pm.id ? 'border-black bg-black' : 'border-gray-300'}`} />
                                ...
                                <div className="mt-4 p-3 bg-orange-50 rounded-xl text-center">
                                    <p className="text-xs text-orange-700 font-bold">
                                        ...
                                        {savings > 0 && <span className="ml-2 text-green-600">(₹{savings.toLocaleString()} saved!)</span>}
                                    </p>
                                </div>
                                <div className="flex gap-3 mt-6">
                                    <button onClick={() => setStep(0)} className="flex-1 border border-gray-200 py-3 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-50 transition">← Back</button>
                                    <button onClick={() => setStep(2)} className="flex-[2] bg-black text-white py-3 rounded-xl font-bold text-sm hover:bg-gray-800 transition flex items-center justify-center gap-2">
                            ...
                            <div className="bg-white rounded-2xl shadow-sm p-6">
                                ...
                                {selectedAddr && (
                                    <div className="mb-4 p-4 bg-gray-50 rounded-xl">
                                        ...
                                        <p className="font-bold text-sm text-gray-800">{selectedAddr.name} — 📞 {selectedAddr.phone}</p>
                                        <p className="text-sm text-gray-600 mt-0.5">{selectedAddr.address}, {selectedAddr.locality}, {selectedAddr.city} — {selectedAddr.pincode}</p>
                                    </div>
                                )}
                                ...
                                <div className="mb-2">
                                    <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Items ({totalItems})</p>
                                    <div className="space-y-3">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-xs text-gray-400">
                                                        ...
                                                        {product.originalPrice > product.discountedPrice && (
                                                            <span className="ml-1 line-through text-gray-300">₹{product.originalPrice}</span>
                                                        )}
                                                    </p>
                                                </div>
                                ...
                                <div className="flex gap-3 mt-6">
                                    <button onClick={() => setStep(1)} className="flex-1 border border-gray-200 py-3 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-50 transition">← Back</button>
                                    <button onClick={handlePlaceOrder} disabled={placing}
                                        ...
                                        {placing ? 'Placing...' : `Place Order • ₹${payableTotal.toLocaleString()}`}
                                    </button>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="lg:w-80">
                        <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-4">
                            <h2 className="font-bold text-gray-700 uppercase text-sm tracking-wide mb-4">Order Summary</h2>
